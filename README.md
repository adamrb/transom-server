...
Three engines, selected with `PB_STT_ENGINE`:
...
Long jobs and proxies: a transcription with enhancement, alternates and consensus takes minutes, and proxies such as a Cloudflare tunnel close a response that has sent nothing for 100 s. Work that outlasts the first `TRANSCRIBE_HEARTBEAT_S` (15 s) is therefore streamed — a whitespace byte every 15 s, then the body — while quick outcomes get a plain response with a real status code. Leading whitespace is legal JSON, and a failure after the first byte arrives as `{"error": ...}` (which the openai engine turns into an error too).
