...
Self-hosted sync target for [Plaud](https://www.plaud.ai) recorders (Note Pro / NotePin S), used together with the [Transom Android app](https://github.com/adamrb/transom-android). Your recordings sync from the device to your phone to your own server, get transcribed by any OpenAI-compatible speech-to-text endpoint you choose, and land as JSON + markdown files you can automate against. No Plaud subscription required.

```
Plaud device ──BLE/WiFi──> Android app ──HTTPS──> transom-server
                               │                        │
...
```bash
git clone https://github.com/adamrb/transom-server.git
cd transom-server
cp .env.example .env
...
## Upgrading from Plaud Bridge

This project was called Plaud Bridge until September 2026. Everything on disk is compatible, but three things need a hand:
...
The server can host the Transom APK itself, so phones install and update the app straight from your own server — no app store. One release is hosted at a time (the latest); its manifest lives at `PB_DATA_DIR/apk/latest.json` and prior APK files stay on disk.
...
Raw data lives under `PB_DATA_DIR`: `recordings/YYYY/MM/<hash>_<name>.mp3` with a sibling `.transcript.json`, and a SQLite index at `transom.sqlite3`.
...
Transom is an independent project, not affiliated with or endorsed by PLAUD Inc. It uses Plaud's public [Embedded SDK / developer platform](https://docs.plaud.ai). Plaud, Plaud Note and NotePin are trademarks of Plaud Inc., used here only to describe compatibility. Apache-2.0 licensed.
